...
// Input handling
void gui_update_input(gui_context_t* ctx, float mouse_x, float mouse_y, 
                      const bool* mouse_buttons, float mouse_wheel, float delta_time);

// =============================================================================
// LAYOUT API
// =============================================================================

void gui_begin_vbox(gui_context_t* ctx, float x, float y, float width, float padding, float spacing);
void gui_end_vbox(gui_context_t* ctx);

void gui_begin_hbox(gui_context_t* ctx, float x, float y, float height, float padding, float spacing);
...
bool gui_slider_float(gui_context_t* ctx, const char* label, float* value, float min, float max, float width);

// =============================================================================
// DRAW API (Low-Level Primitives)
// =============================================================================

void gui_add_rect(gui_context_t* ctx, float x, float y, float w, float h, 
                  gui_color_t color, float thickness);
void gui_add_rect_filled(gui_context_t* ctx, float x, float y, float w, float h, 
                         gui_color_t color);
void gui_add_circle(gui_context_t* ctx, float cx, float cy, float radius, 
                    gui_color_t color, float thickness);
void gui_add_circle_filled(gui_context_t* ctx, float cx, float cy, float radius, 
                           gui_color_t color);
void gui_add_line(gui_context_t* ctx, float x1, float y1, float x2, float y2, 
                  gui_color_t color, float thickness);
void gui_add_triangle_filled(gui_context_t* ctx, float x1, float y1, float x2, float y2, 
                             float x3, float y3, gui_color_t color);

void gui_add_text(gui_context_t* ctx, const char* text, float x, float y, 
                  gui_color_t color, float font_size);
...
static void* gui_alloc(gui_allocator_t* alloc, size_t size) {
    // Align to 8 bytes
    size = (size + 7) & ~7;
...
static void gui_reset_allocator(gui_allocator_t* alloc) {
    alloc->used = 0;
}
...
void gui_init(gui_context_t* ctx) {
    ...
    ctx->allocator.size = CGUI_FRAME_ALLOCATOR_SIZE;
...
static void gui_prim_rect_filled(gui_context_t* ctx, float x, float y, float w, float h, 
    ...
    ctx->vertices[ctx->vertex_count++] = (gui_vertex_t){
        {x, y}, {0, 0}, color
    };
    ctx->vertices[ctx->vertex_count++] = (gui_vertex_t){
        {x + w, y}, {1, 0}, color
...
bool gui_slider_float(gui_context_t* ctx, const char* label, float* value, float min, float max, float width) {
    ...
    if (ctx->active_item == id) {
        if (ctx->input.mouse_down[GUI_MOUSE_BUTTON_LEFT]) {
            ...
            if (mouse_norm < 0.0f) mouse_norm = 0.0f;
            if (mouse_norm > 1.0f) mouse_norm = 1.0f;
    ...
    // Draw slider grab
    gui_color_t grab_color = (ctx->active_item == id) ? ctx->style.slider_grab_active : ctx->style.slider_grab;
    gui_add_circle_filled(ctx, grab_x + grab_w * 0.5f, grab_y + grab_w * 0.5f, grab_w * 0.5f, grab_color);
